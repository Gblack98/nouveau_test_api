...
# ========== Fonction pour reconstruire les fichiers ==========
def rebuild_file_from_parts(parts_dir: Path, filename_prefix: str, final_filepath: Path):
    """
    Reconstitue un fichier à partir de ses parties dans parts_dir,
    en écrivant dans final_filepath.
    Supposé que les fichiers sont nommés filename_prefix.part_aa, .part_ab, etc.
    """
...
async def startup():
    ...
    # Reconstituer les fichiers modèles detection et classification dans /tmp/rebuilt_models
    ...
    try:
        ...
        print(f"Erreur reconstruction modèles feuille: {e}")
        raise
    ...
    try:
        ...
        print(f"Erreur chargement modèle detection feuille: {e}")
        models["detect"] = None
        
    try:
        models["classif"] = load_model(str(classif_model_path))
    except Exception as e:
        print(f"Erreur chargement modèle classification feuille: {e}")
        models["classif"] = None
    
    # Lancer nettoyage asynchrone des fichiers crops
    ...
    for r in results:
        for box in r.boxes:
            ...
            detections.append(DetectionBox(
                ...
                label="feuille",
                crop_url=crop_url,
                classification={"predicted_class": CLASSES_FEUILLE[idx], "confidence": round(conf, 4)}
            ))
